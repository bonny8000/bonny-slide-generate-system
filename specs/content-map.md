...
| Make an invisible exchange auditable | Ordered messages between named actors | "what happens between X and Y", a handshake, an event lifecycle, who calls whom in what order | `event-sequence` | actors as columns + dashed lifelines + one arrow per message + a banded phase | none |
| Establish the parts and the boundaries | Components, one of which contains another | "what is this made of", which layer lives inside which, an architecture overview | `system-anatomy` | peer blocks on a rail + a wider host holding a tinted nested group + numbered connectors | none |
| Make a cross-functional diagnosis legible | An investigation retold across roles | "how we found the cause", detection → analysis → confirmation → fix, an incident review | `role-thread` | role chips in token colours + bubbles + one turn expanded into an evidence card | none |
| Make a recurring practice credible | A ritual or cadence the team actually runs | "the monthly review we set up", a standing meeting and its agenda, the routine that stops recurrence | `practice-hero` | one illustration with role tags + a caption naming the practice + three numbered steps | generate |
